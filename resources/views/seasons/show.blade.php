<x-app-layout>
    ...
    <div class="max-w-4xl space-y-6" x-data="{ tab: @js($initialTab) }">
        ...
        <div x-show="tab === 'menues'" x-cloak class="space-y-6">
            ...
            {{-- Neues Menü anlegen --}}
            <div class="rounded-xl border border-gray-200 bg-white p-6"
                 x-data="{ slots: [{ category_id: '', quantity: 1 }] }">
                <h2 class="text-base font-semibold text-gray-800">Neues Menü</h2>
                <form method="POST" action="{{ route('module.schulkantine.menu-templates.store', $season) }}" class="mt-3 space-y-4">
                    @csrf
                    <div class="grid gap-4 sm:grid-cols-2">
                        <div>
                            ...
                            <x-text-input id="menu_name" name="name" type="text" class="mt-1 block w-full"
                                          :value="old('name')" placeholder="z. B. Menü 1" required />
